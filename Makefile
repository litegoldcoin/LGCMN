...
pkgdatadir = $(datadir)/litegoldcoinmn
pkgincludedir = $(includedir)/litegoldcoinmn
pkglibdir = $(libdir)/litegoldcoinmn
pkglibexecdir = $(libexecdir)/litegoldcoinmn
...
CONFIG_HEADER = $(top_builddir)/src/config/litegoldcoinmn-config.h
...
am__DIST_COMMON = $(srcdir)/Makefile.in \
	...
	$(top_srcdir)/src/config/litegoldcoinmn-config.h.in \
...
ACLOCAL = ${SHELL} /tmp/LiteGoldCoinMN/build-aux/missing aclocal-1.15
AMTAR = $${TAR-tar}
AM_DEFAULT_VERBOSITY = 0
AR = /usr/bin/ar
ARFLAGS = cr
AUTOCONF = ${SHELL} /tmp/LiteGoldCoinMN/build-aux/missing autoconf
AUTOHEADER = ${SHELL} /tmp/LiteGoldCoinMN/build-aux/missing autoheader
AUTOMAKE = ${SHELL} /tmp/LiteGoldCoinMN/build-aux/missing automake-1.15
AWK = gawk
BDB_CFLAGS = 
BDB_CPPFLAGS = 
BDB_LIBS = -ldb_cxx-4.8
BITCOIN_CLI_NAME = litegoldcoinmn-cli
BITCOIN_DAEMON_NAME = litegoldcoinmnd
BITCOIN_GUI_NAME = litegoldcoinmn-qt
BITCOIN_TX_NAME = litegoldcoinmn-tx
...
MAINT = 
MAKEINFO = ${SHELL} /tmp/LiteGoldCoinMN/build-aux/missing makeinfo
MAKENSIS = 
MANIFEST_TOOL = :
MINIUPNPC_CPPFLAGS = 
...
OTOOL = 
OTOOL64 = 
PACKAGE = litegoldcoinmn
PACKAGE_BUGREPORT = www.litegoldcoinmn.org
PACKAGE_NAME = LiteGoldCoinMN Core
PACKAGE_STRING = LiteGoldCoinMN Core 1.0.0
PACKAGE_TARNAME = litegoldcoinmn
PACKAGE_URL = 
...
ZMQ_CFLAGS = 
ZMQ_LIBS = -lzmq
abs_builddir = /tmp/LiteGoldCoinMN
abs_srcdir = /tmp/LiteGoldCoinMN
abs_top_builddir = /tmp/LiteGoldCoinMN
abs_top_srcdir = /tmp/LiteGoldCoinMN
ac_ct_AR = ar
ac_ct_CC = gcc
ac_ct_CXX = g++
ac_ct_DUMPBIN = 
...
install_sh = ${SHELL} /tmp/LiteGoldCoinMN/build-aux/install-sh
...
COVERAGE_INFO = baseline_filtered_combined.info baseline.info block_test.info \
  leveldb_baseline.info test_litegoldcoinmn_filtered.info total_coverage.info \
  baseline_filtered.info block_test_filtered.info \
  leveldb_baseline_filtered.info test_litegoldcoinmn_coverage.info test_litegoldcoinmn.info

OSX_APP_BUILT = $(OSX_APP)/Contents/PkgInfo $(OSX_APP)/Contents/Resources/empty.lproj \
  $(OSX_APP)/Contents/Resources/bitcoin.icns $(OSX_APP)/Contents/Info.plist \
  $(OSX_APP)/Contents/MacOS/LiteGoldCoinMN-Qt $(OSX_APP)/Contents/Resources/Base.lproj/InfoPlist.strings
...
src/config/litegoldcoinmn-config.h: src/config/stamp-h1
	@test -f $@ || rm -f src/config/stamp-h1
	@test -f $@ || $(MAKE) $(AM_MAKEFLAGS) src/config/stamp-h1

src/config/stamp-h1: $(top_srcdir)/src/config/litegoldcoinmn-config.h.in $(top_builddir)/config.status
	@rm -f src/config/stamp-h1
	cd $(top_builddir) && $(SHELL) ./config.status src/config/litegoldcoinmn-config.h
$(top_srcdir)/src/config/litegoldcoinmn-config.h.in:  $(am__configure_deps) 
	($(am__cd) $(top_srcdir) && $(AUTOHEADER))
	rm -f src/config/stamp-h1
	touch $@

distclean-hdr:
	-rm -f src/config/litegoldcoinmn-config.h src/config/stamp-h1
...
$(OSX_APP)/Contents/MacOS/LiteGoldCoinMN-Qt: $(BITCOIN_QT_BIN)
	$(MKDIR_P) $(@D)
...
$(APP_DIST_EXTRAS): $(APP_DIST_DIR)/$(OSX_APP)/Contents/MacOS/LiteGoldCoinMN-Qt
...
$(APP_DIST_DIR)/$(OSX_APP)/Contents/MacOS/LiteGoldCoinMN-Qt: $(OSX_APP_BUILT) $(OSX_PACKAGING)
...
#test_litegoldcoinmn.info: baseline_filtered_combined.info
#	$(MAKE) -C src/ check
#	$(LCOV) -c -d $(abs_builddir)/src -t test_litegoldcoinmn -o $@
#	$(LCOV) -z -d $(abs_builddir)/src
#	$(LCOV) -z -d $(abs_builddir)/src/leveldb

#test_litegoldcoinmn_filtered.info: test_litegoldcoinmn.info
#	$(LCOV) -r $< "/usr/include/*" -o $@

#block_test.info: test_litegoldcoinmn_filtered.info
#	$(MKDIR_P) qa/tmp
...
#test_litegoldcoinmn_coverage.info: baseline_filtered_combined.info test_litegoldcoinmn_filtered.info
#	$(LCOV) -a baseline_filtered.info -a leveldb_baseline_filtered.info -a test_litegoldcoinmn_filtered.info -o $@

#total_coverage.info:  baseline_filtered_combined.info test_litegoldcoinmn_filtered.info block_test_filtered.info
#	$(LCOV) -a baseline_filtered.info -a leveldb_baseline_filtered.info -a test_litegoldcoinmn_filtered.info -a block_test_filtered.info -o $@ | $(GREP) "\%" | $(AWK) '{ print substr($$3,2,50) "/" $$5 }' > coverage_percent.txt

#test_litegoldcoinmn.coverage/.dirstamp:  test_litegoldcoinmn_coverage.info
#	$(GENHTML) -s $< -o $(@D)
#	@touch $@
...
#cov: test_litegoldcoinmn.coverage/.dirstamp total.coverage/.dirstamp

.INTERMEDIATE: $(COVERAGE_INFO)

clean-local:
	rm -rf test_litegoldcoinmn.coverage/ total.coverage/ $(OSX_APP)
