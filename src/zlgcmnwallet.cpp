...
CzLGCMNWallet::CzLGCMNWallet(std::string strWalletFile)
{
    ...
    //Check for old db version of storing zlgcmn seed
    if (fFirstRun) {
        uint256 seed;
        if (walletdb.ReadZLGCMNSeed_deprecated(seed)) {
            //Update to new format, erase old
            seedMaster = seed;
            hashSeed = Hash(seed.begin(), seed.end());
            if (pwalletMain->AddDeterministicSeed(seed)) {
                if (walletdb.EraseZLGCMNSeed_deprecated()) {
                    LogPrintf("%s: Updated zLGCMN seed databasing\n", __func__);
                    fFirstRun = false;
                } else {
                    LogPrintf("%s: failed to remove old zlgcmn seed\n", __func__);
                }
            }
        }
    }
    ...
    if (fFirstRun) {
        ...
        seedMaster = seed;
        LogPrintf("%s: first run of zlgcmn wallet detected, new seed generated. Seedhash=%s\n", __func__, Hash(seed.begin(), seed.end()).GetHex());
...
bool CzLGCMNWallet::SetMasterSeed(const uint256& seedMaster, bool fResetCount)
{
    ...
    if (fResetCount)
        walletdb.WriteZLGCMNCount(nCountLastUsed);
    else if (!walletdb.ReadZLGCMNCount(nCountLastUsed))
        nCountLastUsed = 0;
...
void CzLGCMNWallet::Lock()
{
    seedMaster = 0;
}

void CzLGCMNWallet::AddToMintPool(const std::pair<uint256, uint32_t>& pMint, bool fVerbose)
{
    mintPool.Add(pMint, fVerbose);
}

//Add the next 20 mints to the mint pool
void CzLGCMNWallet::GenerateMintPool(uint32_t nCountStart, uint32_t nCountEnd)
{
    ...
    for (uint32_t i = n; i < nStop; ++i) {
        ...
        CKey key;
        SeedToZLGCMN(seedZerocoin, bnValue, bnSerial, bnRandomness, key);
...
bool CzLGCMNWallet::LoadMintPoolFromDB()
{
...
void CzLGCMNWallet::RemoveMintsFromPool(const std::vector<uint256>& vPubcoinHashes)
{
    for (const uint256& hash : vPubcoinHashes)
        mintPool.Remove(hash);
}

void CzLGCMNWallet::GetState(int& nCount, int& nLastGenerated)
{
...
void CzLGCMNWallet::SyncWithChain(bool fGenerateMintPool)
{
    ...
    while (found) {
        ...
        for (pair<uint256, uint32_t> pMint : listMints) {
            ...
            if (pwalletMain->zlgcmnTracker->HasPubcoinHash(pMint.first)) {
...
bool CzLGCMNWallet::SetMintSeen(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const CoinDenomination& denom)
{
    ...
    CBigNum bnRandomness;
    CKey key;
    SeedToZLGCMN(seedZerocoin, bnValueGen, bnSerial, bnRandomness, key);
    ...
    // Add to zlgcmnTracker which also adds to database
    pwalletMain->zlgcmnTracker->Add(dMint, true);
...
void CzLGCMNWallet::SeedToZLGCMN(const uint512& seedZerocoin, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key)
{
...
uint512 CzLGCMNWallet::GetZerocoinSeed(uint32_t n)
{
...
void CzLGCMNWallet::UpdateCount()
{
    nCountLastUsed++;
    CWalletDB walletdb(strWalletFile);
    walletdb.WriteZLGCMNCount(nCountLastUsed);
...
void CzLGCMNWallet::GenerateDeterministicZLGCMN(CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly)
{
...
void CzLGCMNWallet::GenerateMint(const uint32_t& nCount, const CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint)
{
...
bool CzLGCMNWallet::RegenerateMint(const CDeterministicMint& dMint, CZerocoinMint& mint)
{
