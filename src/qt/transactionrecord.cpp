...
QList<TransactionRecord> TransactionRecord::decomposeTransaction(const CWallet* wallet, const CWalletTx& wtx)
{
    ...
    if (wtx.IsCoinStake()) {
        ...
        if (wtx.IsZerocoinSpend() && (fZSpendFromMe || wallet->zlgcmnTracker->HasMintTx(hash))) {
            //zLGCMN stake reward
            sub.involvesWatchAddress = false;
            sub.type = TransactionRecord::StakeZLGCMN;
        ...
        BOOST_FOREACH (const CTxOut& txout, wtx.vout) {
            ...
            if (mine) {
                ...
                if (ExtractDestination(txout.scriptPubKey, address) && IsMine(*wallet, address)) {
                    // Received by LiteGoldCoinMN Address
        ...
        if (fAllFromMeDenom && fAllToMeDenom && nFromMe * nToMe) {
            ...
            if (mapValue["DS"] == "1") {
                ...
                if (ExtractDestination(wtx.vout[0].scriptPubKey, address)) {
                    // Sent to LiteGoldCoinMN Address
                    sub.address = CBitcoinAddress(address).ToString();
                } else {
...
bool IsZLGCMNType(TransactionRecord::Type type)
{
    switch (type) {
        case TransactionRecord::StakeZLGCMN:
...
void TransactionRecord::updateStatus(const CWalletTx& wtx)
{
    ...
    else if (type == TransactionRecord::Generated || type == TransactionRecord::StakeMint || type == TransactionRecord::StakeZLGCMN || type == TransactionRecord::MNReward) {
