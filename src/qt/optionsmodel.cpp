...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nDisplayUnit"))
        settings.setValue("nDisplayUnit", BitcoinUnits::LGCMN);
    ...
    if (!settings.contains("nAnonymizeLiteGoldCoinMNAmount"))
        settings.setValue("nAnonymizeLiteGoldCoinMNAmount", 1000);

    nAnonymizeLiteGoldCoinMNAmount = settings.value("nAnonymizeLiteGoldCoinMNAmount").toLongLong();
...
// Wallet
    ...
    if (settings.contains("nAnonymizeLiteGoldCoinMNAmount"))
        SoftSetArg("-anonymizelitegoldcoinmnamount", settings.value("nAnonymizeLiteGoldCoinMNAmount").toString().toStdString());
...
void OptionsModel::Reset()
{
    ...
    settings.clear();
    resetSettings = true; // Needed in litegoldcoinmn.cpp during shotdown to also remove the window positions
...
QVariant OptionsModel::data(const QModelIndex& index, int role) const
{
    if (role == Qt::EditRole) {
        ...
        case AnonymizeLiteGoldCoinMNAmount:
            return QVariant(nAnonymizeLiteGoldCoinMNAmount);
        case Listen:
...
bool OptionsModel::setData(const QModelIndex& index, const QVariant& value, int role)
{
    ...
    if (role == Qt::EditRole) {
        ...
        case AnonymizeLiteGoldCoinMNAmount:
            nAnonymizeLiteGoldCoinMNAmount = value.toInt();
            settings.setValue("nAnonymizeLiteGoldCoinMNAmount", nAnonymizeLiteGoldCoinMNAmount);
            emit anonymizeLiteGoldCoinMNAmountChanged(nAnonymizeLiteGoldCoinMNAmount);
            break;
        case CoinControlFeatures:
