...
BOOST_AUTO_TEST_CASE(zerocoin_spend_test)
{
    ...
    cWallet.zlgcmnTracker = unique_ptr<CzLGCMNTracker>(new CzLGCMNTracker(cWallet.strWalletFile));
    ...
    BOOST_CHECK_MESSAGE(receipt.GetStatus() == ZLGCMN_TRX_FUNDS_PROBLEMS, "Failed Invalid Amount Check");
    ...
    BOOST_CHECK_MESSAGE(receipt2.GetStatus() == ZLGCMN_TRX_FUNDS_PROBLEMS, "Failed Invalid Amount Check");
