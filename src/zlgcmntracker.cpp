...
CzLGCMNTracker::CzLGCMNTracker(std::string strWalletFile)
{
...
CzLGCMNTracker::~CzLGCMNTracker()
{
    mapSerialHashes.clear();
    mapPendingSpends.clear();
}

void CzLGCMNTracker::Init()
{
...
bool CzLGCMNTracker::Archive(CMintMeta& meta)
{
...
bool CzLGCMNTracker::UnArchive(const uint256& hashPubcoin, bool isDeterministic)
{
    CWalletDB walletdb(strWalletFile);
    if (isDeterministic) {
        CDeterministicMint dMint;
...
CMintMeta CzLGCMNTracker::Get(const uint256 &hashSerial)
{
...
CMintMeta CzLGCMNTracker::GetMetaFromPubcoin(const uint256& hashPubcoin)
{
...
bool CzLGCMNTracker::GetMetaFromStakeHash(const uint256& hashStake, CMintMeta& meta) const
{
    for (auto& it : mapSerialHashes) {
...
std::vector<uint256> CzLGCMNTracker::GetSerialHashes()
{
...
CAmount CzLGCMNTracker::GetBalance(bool fConfirmedOnly, bool fUnconfirmedOnly) const
{
    CAmount nTotal = 0;
...
CAmount CzLGCMNTracker::GetUnconfirmedBalance() const
{
    return GetBalance(false, true);
}

std::vector<CMintMeta> CzLGCMNTracker::GetMints(bool fConfirmedOnly) const
{
...
bool CzLGCMNTracker::HasMintTx(const uint256& txid)
{
    for (auto it : mapSerialHashes) {
...
bool CzLGCMNTracker::HasPubcoin(const CBigNum &bnValue) const
{
...
bool CzLGCMNTracker::HasPubcoinHash(const uint256& hashPubcoin) const
{
    for (auto it : mapSerialHashes) {
        CMintMeta meta = it.second;
        if (meta.hashPubcoin == hashPubcoin)
...
bool CzLGCMNTracker::HasSerial(const CBigNum& bnSerial) const
{
    uint256 hash = GetSerialHash(bnSerial);
    return HasSerialHash(hash);
}

bool CzLGCMNTracker::HasSerialHash(const uint256& hashSerial) const
{
    auto it = mapSerialHashes.find(hashSerial);
    return it != mapSerialHashes.end();
}

bool CzLGCMNTracker::UpdateZerocoinMint(const CZerocoinMint& mint)
{
...
bool CzLGCMNTracker::UpdateState(const CMintMeta& meta)
{
    CWalletDB walletdb(strWalletFile);
...
void CzLGCMNTracker::Add(const CDeterministicMint& dMint, bool isNew, bool isArchived)
{
    CMintMeta meta;
...
void CzLGCMNTracker::Add(const CZerocoinMint& mint, bool isNew, bool isArchived)
{
    CMintMeta meta;
...
void CzLGCMNTracker::SetPubcoinUsed(const uint256& hashPubcoin, const uint256& txid)
{
...
void CzLGCMNTracker::SetPubcoinNotUsed(const uint256& hashPubcoin)
{
    if (!HasPubcoinHash(hashPubcoin))
        return;
    CMintMeta meta = GetMetaFromPubcoin(hashPubcoin);
...
void CzLGCMNTracker::RemovePending(const uint256& txid)
{
    uint256 hashSerial;
...
bool CzLGCMNTracker::UpdateStatusInternal(const std::set<uint256>& setMempool, CMintMeta& mint)
{
...
std::set<CMintMeta> CzLGCMNTracker::ListMints(bool fUnusedOnly, bool fMatureOnly, bool fUpdateStatus)
{
    CWalletDB walletdb(strWalletFile);
    if (fUpdateStatus) {
        ...
        for (auto& dMint : listDeterministicDB)
            Add(dMint);
        LogPrint("zero", "%s: added %d dzlgcmn from DB\n", __func__, listDeterministicDB.size());
    }
