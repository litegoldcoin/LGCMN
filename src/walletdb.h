...
public:
    ...
    bool WriteZLGCMNSeed(const uint256& hashSeed, const vector<unsigned char>& seed);
    bool ReadZLGCMNSeed(const uint256& hashSeed, vector<unsigned char>& seed);
    bool ReadZLGCMNSeed_deprecated(uint256& seed);
    bool EraseZLGCMNSeed();
    bool EraseZLGCMNSeed_deprecated();

    bool WriteZLGCMNCount(const uint32_t& nCount);
    bool ReadZLGCMNCount(uint32_t& nCount);
