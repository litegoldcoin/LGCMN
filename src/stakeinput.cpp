...
// The zLGCMN block index is the first appearance of the accumulator checksum that was used in the spend
...
{
    //The unique identifier for a zLGCMN is a hash of the serial
...
{
    //Create an output returning the zLGCMN that was staked
    CTxOut outReward;
    libzerocoin::CoinDenomination denomStaked = libzerocoin::AmountToZerocoinDenomination(this->GetValue());
    CDeterministicMint dMint;
    if (!pwallet->CreateZLGCMNOutPut(denomStaked, outReward, dMint))
        return error("%s: failed to create zLGCMN output", __func__);
    vout.emplace_back(outReward);

    //Add new staked denom to our wallet
    if (!pwallet->DatabaseMint(dMint))
        return error("%s: failed to database the staked zLGCMN", __func__);

    for (unsigned int i = 0; i < 3; i++) {
        CTxOut out;
        CDeterministicMint dMintReward;
        if (!pwallet->CreateZLGCMNOutPut(libzerocoin::CoinDenomination::ZQ_ONE, out, dMintReward))
            return error("%s: failed to create zLGCMN output", __func__);
...
{
    CzLGCMNTracker* zlgcmnTracker = pwallet->zlgcmnTracker.get();
    CMintMeta meta;
    if (!zlgcmnTracker->GetMetaFromStakeHash(hashSerial, meta))
        return error("%s: tracker does not have serialhash", __func__);

    zlgcmnTracker->SetPubcoinUsed(meta.hashPubcoin, txid);
    return true;
}
...
{
    //The unique identifier for a LGCMN stake is the outpoint
