...
#include "config/litegoldcoinmn-config.h"
#endif
...
CzLGCMNWallet* zwalletMain = NULL;
...
void PrepareShutdown()
{
    ...
    RenameThread("litegoldcoinmn-shutoff");
...
std::string HelpMessage(HelpMessageMode mode)
{
    ...
    strUsage += HelpMessageOpt("-conf=<file>", strprintf(_("Specify configuration file (default: %s)"), "litegoldcoinmn.conf"));
...
#ifndef WIN32
    strUsage += HelpMessageOpt("-pid=<file>", strprintf(_("Specify pid file (default: %s)"), "litegoldcoinmnd.pid"));
#endif
    strUsage += HelpMessageOpt("-reindex", _("Rebuild block chain index from current blk000??.dat files") + " " + _("on startup"));
    strUsage += HelpMessageOpt("-reindexaccumulators", _("Reindex the accumulator database") + " " + _("on startup"));
    strUsage += HelpMessageOpt("-reindexmoneysupply", _("Reindex the LGCMN and zLGCMN money supply statistics") + " " + _("on startup"));
    ...
    strUsage += HelpMessageOpt("-port=<port>", strprintf(_("Listen for connections on <port> (default: %u or testnet: %u)"), 8248, 18248));
    ...
    if (GetBoolArg("-help-debug", false))
        strUsage += HelpMessageOpt("-mintxfee=<amt>", strprintf(_("Fees (in LGCMN/Kb) smaller than this are considered zero fee for transaction creation (default: %s)"),
            FormatMoney(CWallet::minTxFee.GetFeePerK())));
    strUsage += HelpMessageOpt("-paytxfee=<amt>", strprintf(_("Fee (in LGCMN/kB) to add to transactions you send (default: %s)"), FormatMoney(payTxFee.GetFeePerK())));
    ...
    string debugCategories = "addrman, alert, bench, coindb, db, lock, rand, rpc, selectcoins, tor, mempool, net, proxy, http, libevent, litegoldcoinmn, (obfuscation, swiftx, masternode, mnpayments, mnbudget, zero)"; // Don't translate these and qt below
    if (mode == HMM_BITCOIN_QT)
        debugCategories += ", qt";
    ...
    strUsage += HelpMessageOpt("-minrelaytxfee=<amt>", strprintf(_("Fees (in LGCMN/Kb) smaller than this are considered zero fee for relaying (default: %s)"), FormatMoney(::minRelayTxFee.GetFeePerK())));
    ...
    strUsage += HelpMessageOpt("-litemode=<n>", strprintf(_("Disable all LiteGoldCoinMN specific functionality (Masternodes, Zerocoin, SwiftX, Budgeting) (0-1, default: %u)"), 0));

#ifdef ENABLE_WALLET
    strUsage += HelpMessageGroup(_("Staking options:"));
    strUsage += HelpMessageOpt("-staking=<n>", strprintf(_("Enable staking functionality (0-1, default: %u)"), 1));
    strUsage += HelpMessageOpt("-lgcmnstake=<n>", strprintf(_("Enable or disable staking functionality for LGCMN inputs (0-1, default: %u)"), 1));
    strUsage += HelpMessageOpt("-zlgcmnstake=<n>", strprintf(_("Enable or disable staking functionality for zLGCMN inputs (0-1, default: %u)"), 1));
    ...
    strUsage += HelpMessageOpt("-masternodeaddr=<n>", strprintf(_("Set external address:port to get to this masternode (example: %s)"), "128.127.106.235:8248"));
    ...
    strUsage += HelpMessageOpt("-backupzlgcmn=<n>", strprintf(_("Enable automatic wallet backups triggered after each zLGCMN minting (0-1, default: %u)"), 1));
    strUsage += HelpMessageOpt("-zlgcmnbackuppath=<dir|file>", _("Specify custom backup path to add a copy of any automatic zLGCMN backup. If set as dir, every backup generates a timestamped file. If set as file, will rewrite to that file every backup. If backuppath is set as well, 4 backups will happen"));
#endif // ENABLE_WALLET
...
//    strUsage += "  -anonymizelitegoldcoinmnamount=<n>     " + strprintf(_("Keep N LGCMN anonymized (default: %u)"), 0) + "\n";
//    strUsage += "  -liquidityprovider=<n>       " + strprintf(_("Provide liquidity to Obfuscation by infrequently mixing coins on a continual basis (0-100, default: %u, 1=very frequent, high fees, 100=very infrequent, low fees)"), 0) + "\n";
    ...
    strUsage += HelpMessageOpt("-rpcport=<port>", strprintf(_("Listen for JSON-RPC connections on <port> (default: %u or testnet: %u)"), 8247, 18247));
...
std::string LicenseInfo()
{
    return FormatParagraph(strprintf(_("Copyright (C) 2009-%i The Bitcoin Core Developers"), COPYRIGHT_YEAR)) + "\n" +
           ...
           "\n" +
           FormatParagraph(strprintf(_("Copyright (C) %i The LiteGoldCoinMN Core Developers"), COPYRIGHT_YEAR)) + "\n" +
           "\n" +
...
{
    RenameThread("litegoldcoinmn-loadblk");
...
/** Sanity checks
 *  Ensure that LiteGoldCoinMN is running in a usable environment with all
...
#define PROCESS_DEP_ENABLE 0x00000001
    ...
    // Sanity check
    if (!InitSanityCheck())
        return InitError(_("Initialization sanity check failed. LiteGoldCoinMN Core is shutting down."));
...
#endif
    // Make sure only a single LiteGoldCoinMN process is using the data directory.
    ...
    if (!lock.timed_lock(boost::get_system_time() + boost::posix_time::seconds(10)))
        return InitError(strprintf(_("Cannot obtain a lock on data directory %s. LiteGoldCoinMN Core is probably already running."), strDataDir));
    ...
    if (GetBoolArg("-shrinkdebugfile", !fDebug))
        ShrinkDebugFile();
    LogPrintf("\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n");
    LogPrintf("LiteGoldCoinMN version %s (%s)\n", FormatFullVersion(), CLIENT_DATE);
...
// ********************************************************* Step 5: Backup wallet and verify wallet database integrity
    ...
    //LiteGoldCoinMN: Load Accumulator Checkpoints according to network (main/test/regtest)
    ...
    while (!fLoaded) {
        ...
        do {
            try {
                ...
                //LiteGoldCoinMN specific: zerocoin and spork DB's
                ...
                // LiteGoldCoinMN: load previous sessions sporks if we have them.
                ...
                if (GetBoolArg("-reindexmoneysupply", false)) {
                    if (chainActive.Height() > Params().Zerocoin_StartHeight()) {
                        RecalculateZLGCMNMinted();
                        RecalculateZLGCMNSpent();
                    }
                    RecalculateLGCMNSupply(1);
                }
                ...
                if (GetBoolArg("-reindexaccumulators", false)) {
                    if (chainActive.Height() > Params().Zerocoin_Block_V2_Start()) {
                        ...
                        // LiteGoldCoinMN: recalculate Accumulator Checkpoints that failed to database properly
    ...
    if (fDisableWallet) {
        ...
        if (nLoadWalletRet != DB_LOAD_OK) {
            if (nLoadWalletRet == DB_CORRUPT)
                ...
                strErrors << _("Error loading wallet.dat: Wallet requires newer version of LiteGoldCoinMN Core") << "\n";
            else if (nLoadWalletRet == DB_NEED_REWRITE) {
                strErrors << _("Wallet needed to be rewritten: restart LiteGoldCoinMN Core to complete") << "\n";
        ...
        zwalletMain = new CzLGCMNWallet(pwalletMain->strWalletFile);
        ...
        //Inititalize zLGCMNWallet
        uiInterface.InitMessage(_("Syncing zLGCMN wallet..."));

        bool fEnableZPivBackups = GetBoolArg("-backupzlgcmn", true);
        pwalletMain->setZPivAutoBackups(fEnableZPivBackups);

        //Load zerocoin mint hashes to memory
        pwalletMain->zlgcmnTracker->Init();
...
// XX42 Remove/refactor code below. Until then provide safe defaults
    nAnonymizeLiteGoldCoinMNAmount = 2;
...
//    }
//
//    nAnonymizeLiteGoldCoinMNAmount = GetArg("-anonymizelitegoldcoinmnamount", 0);
//    if (nAnonymizeLiteGoldCoinMNAmount > 999999) nAnonymizeLiteGoldCoinMNAmount = 999999;
//    if (nAnonymizeLiteGoldCoinMNAmount < 2) nAnonymizeLiteGoldCoinMNAmount = 2;
    ...
    LogPrintf("Anonymize LiteGoldCoinMN Amount %d\n", nAnonymizeLiteGoldCoinMNAmount);
    ...
    /* Denominations
       ...
       For example:
       1LGCMN+1000 == (.1LGCMN+100)*10
       10LGCMN+10000 == (1LGCMN+1000)*10
    */
